...
星河岛 **1.1** 接入库 **1.1.0**，支持消息、进度、计时、音乐和自绘卡片；同时发现独立星河岛与星流内置宿主。

采用 [PolyForm Noncommercial 1.0.0](LICENSE.md)：禁止商用，允许闭源接入、修改和分发。分发时保留许可与版权声明。

## 开始接入

1. 从[发布页](https://github.com/MuYuanXing/AstraIsland-Developers/releases)下载并安装星河岛1.1，启用系统模块；或安装带星河岛能力的星流。
2. 下载 `sdk/astraisland-client-1.1.0.aar`，使用 `SHA256SUMS` 校验。
